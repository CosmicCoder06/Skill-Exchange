...
function App() {
    ...
    function handleLogin(newToken) { localStorage.setItem("Token", newToken); setProfileStatus(getTokenRole(newToken) === "admin" ? true : null); setPage("home"); setToken(newToken); }
    function handleLogout() { localStorage.removeItem("Token"); setProfileStatus(null); setPage("home"); setToken(null); }
    ...
    return <SocketProvider key={token} token={token}>
        {profileStatus === false ? <CompleteProfile token={token} onComplete={() => { setProfileStatus(true); setPage("profile"); }} onLater={() => { setProfileStatus("skipped"); setPage("profile"); }} />
            : page === "home" ? <HomePage isAdmin={isAdmin} onAdmin={() => setPage("admin")} onDiscover={() => setPage("discover")} onProfile={() => setPage("profile")} onMessages={() => setPage("chat")} />
                : page === "discover" ? <DiscoverPage token={token} onHome={() => setPage("home")} onProfile={() => setPage("profile")} onMessages={() => setPage("chat")} />
                    : page === "admin" && isAdmin ? <AdminDashboard token={token} onHome={() => setPage("home")} onLogout={handleLogout} />
